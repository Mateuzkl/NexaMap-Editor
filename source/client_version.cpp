...
void ClientVersion::loadVersions() {
	...
	try {
		const auto versions = nlohmann::json::parse(g_settings.getString(Config::ASSETS_DATA_DIRS));
		for (const auto& versionInfo : versions) {
			ClientVersion* version = get(versionInfo.at("id").get<std::string>());
			if (version == nullptr) {
				continue;
			}
			version->setClientPath(wxstr(versionInfo.at("path").get<std::string>()));
		}
	...
	if (directory.GetFirst(&otfiFile, "*.otfi", wxDIR_FILES)) {
		...
		try {
			...
			if (document && document->size() != 0 && document->hasChildAt("DatSpr")) {
				...
				error = wxString::Format("The OTML file '%s' does not contain a valid 'DatSpr' configuration.", otfi.GetFullPath());
				return nullptr;
			}
		} catch (const std::exception& e) {
			error = wxString::Format("Failed to parse OTML file %s: %s", otfi.GetFullPath(), wxString::FromUTF8(e.what()));
			return nullptr;
		} catch (...) {
