...
enum class Packet : uint8_t {
    Challenge = 1, Hello, Welcome, SnapshotBegin, SnapshotChunk, SnapshotEnd,
    TransactionBegin, TransactionChunk, TransactionEnd, Accepted, Rejected,
    Resume, Ready, Chat, Cursor, Ping, Pong, Players, Lock, Unlock, Locks,
    LocationPing, Approval, ApprovalResult, Disconnect
};
enum class Role : uint8_t { Host, Editor, Reviewer, Viewer };
...
class Writer {
public:
    explicit Writer(size_t limit = MaxPacket) : limit(limit) {}
    void u8(uint8_t v);
    void u16(uint16_t v);
    void u32(uint32_t v);
    void u64(uint64_t v);
...
public:
    ...
    const std::vector<Lease>& all() const { return leases; }
private:
...
class Revisions {
public:
    std::string validate(const Transaction& tx, const Identity& session, Role role,
        uint32_t author, const Locks& locks, uint64_t now) const;
    uint64_t commit(const Transaction& tx);
    uint64_t current() const { return revision; }
...
struct JournalEntry { uint64_t revision; Bytes bytes; };
class Journal {
public:
    void push(uint64_t revision, Bytes bytes);
    bool covers(uint64_t after, uint64_t current) const;
    const std::deque<JournalEntry>& entries() const { return records; }
    size_t sizeBytes() const { return bytes; }
private:
    size_t bytes = 0;
