...
namespace {
bool IsCurrentMapTab(MapTab* mapTab) {
	MapTab* currentTab = g_gui.GetCurrentMapTab();
	return mapTab && currentTab && mapTab->HasSameReference(currentTab);
}
}
...
void DuplicatedItemsWindow::UpdateButtons() {
	...
	bool enable = items_list->GetCount() != 0 && IsCurrentMapTab(map_tab);
