...
PositionCtrl::PositionCtrl(wxWindow* parent, const wxString& label, int x, int y, int z, int maxx /*= MAP_MAX_WIDTH*/, int maxy /*= MAP_MAX_HEIGHT*/, int maxz /*= MAP_MAX_LAYER*/) :
	...
	wxStaticBox* contentParent = GetStaticBox();
	x_field = newd NumberTextCtrl(contentParent, wxID_ANY, x, 0, maxx, wxTE_PROCESS_ENTER, "X", wxDefaultPosition, wxSize(60, 20));
	x_field->Bind(wxEVT_TEXT_PASTE, &PositionCtrl::OnClipboardText, this);
	Add(x_field, 2, wxEXPAND | wxLEFT | wxBOTTOM, 5);

	y_field = newd NumberTextCtrl(contentParent, wxID_ANY, y, 0, maxy, wxTE_PROCESS_ENTER, "Y", wxDefaultPosition, wxSize(60, 20));
	y_field->Bind(wxEVT_TEXT_PASTE, &PositionCtrl::OnClipboardText, this);
	Add(y_field, 2, wxEXPAND | wxLEFT | wxBOTTOM, 5);

	z_field = newd NumberTextCtrl(contentParent, wxID_ANY, z, 0, maxz, wxTE_PROCESS_ENTER, "Z", wxDefaultPosition, wxSize(35, 20));
